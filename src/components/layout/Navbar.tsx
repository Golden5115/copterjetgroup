    ...
    megaMenu: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'Aircraft Management', href: '/services/management' },
        ],
      },
      {
...
export default function Navbar() {
  ...
  return (
    <header className="w-full relative z-50">
      ...
      {/* ── Top Bar ── */}
      <div className="bg-copter-blue">
        {/* Strictly matched padding (px-6 lg:px-12) to align with the Hero Section */}
        <div className="max-w-7xl mx-auto px-6 lg:px-12 h-10 flex items-center justify-between">
          ...
          <div className="flex items-center gap-5 ml-auto">
            ...
            <div className="hidden md:flex items-center gap-2">
              {[
                { label: 'Facebook', hoverClass: 'hover:bg-[#3b5998]', path: 'M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z' },
                { label: 'Instagram', hoverClass: 'hover:bg-[#e1306c]', path: 'M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z' },
                { label: 'LinkedIn', hoverClass: 'hover:bg-[#0077b5]', path: 'M4.98 3.5c0 1.381-1.11 2.5-2.48 2.5s-2.48-1.119-2.48-2.5c0-1.38 1.11-2.5 2.48-2.5s2.48 1.12 2.48 2.5zm.02 4.5h-5v16h5v-16zm7.982 0h-4.968v16h4.969v-8.399c0-4.67 6.029-5.052 6.029 0v8.399h4.988v-10.131c0-7.88-8.922-7.593-11.018-3.714v-2.155z' },
                { label: 'X', hoverClass: 'hover:bg-[#000]', path: 'M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z' },
              ].map(s => (
                <a key={s.label} href="#" aria-label={s.label}
                  className={`w-6 h-6 flex items-center justify-center rounded-[2px] bg-white/10 text-white hover:text-white transition-all duration-300 ${s.hoverClass}`}
                >
      ...
      {/* ── Main Nav ── */}
      <nav className="bg-white w-full relative z-50 transition-shadow duration-300" style={{ boxShadow: scrolled ? '0 4px 24px rgba(22,72,120,0.12)' : '0 2px 8px rgba(22,72,120,0.06)' }}>
        <div className="absolute top-0 left-0 right-0 h-[3px] bg-copter-red" />

        {/* Strictly matched padding (px-6 lg:px-12) to align with the Hero Section */}
        <div className="max-w-7xl mx-auto px-6 lg:px-12 flex justify-between items-center h-[76px]">
          ...
          <div className="hidden lg:flex items-center h-full">
            {navStructure.map(item => (
              <div key={item.label} className="relative group h-full flex items-center">
                ...
                >
                  ...
                  {/* Premium Animated Sliding Underline */}
            ...
            <Link href="/rfq" className="ml-4 bg-copter-red text-white text-[11px] font-bold tracking-[0.12em] uppercase px-6 py-3 hover:bg-copter-red-dark hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 flex-shrink-0">
              SUBMIT RFQ
            </Link>
          </div>
        ...
        <div className={`lg:hidden absolute w-full bg-white z-40 overflow-y-auto border-t border-gray-100 shadow-[0_12px_40px_rgba(22,72,120,0.15)] transition-all duration-500 ease-in-out origin-top ${mobileOpen ? 'max-h-[82vh] opacity-100' : 'max-h-0 opacity-0 pointer-events-none'}`}>
          <div className="flex flex-col text-copter-blue font-bold">
            ...
            <div className="px-6 py-6 bg-gray-50">
              <Link href="/rfq" className="block text-center bg-copter-red text-white font-bold tracking-[0.15em] text-[12px] py-4 uppercase hover:bg-copter-red-dark transition-colors rounded-[2px]" onClick={() => setMobileOpen(false)}>
                SUBMIT RFQ
              </Link>
            </div>
          </div>
