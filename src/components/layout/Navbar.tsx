  ...
  return (
    <header className="w-full relative z-50">
      ...
      <div className="bg-copter-blue">
        ...
        <div className="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-12 h-9 flex items-center justify-between">
          <span className="hidden md:block text-[10px] font-semibold tracking-[0.2em] uppercase" style={{ color: 'rgb(255, 255, 255)' }}>
          ...
          <div className="flex items-center gap-5 ml-auto">
            {['News & Events', 'Gallery', 'Contact Us'].map(lbl => (
              <Link key={lbl} href={`/${lbl.toLowerCase().replace(/ & /g, '-').replace(/ /g, '-')}`}
                className="hidden sm:block text-[10px] font-bold tracking-widest uppercase transition-colors"
                style={{ color: 'rgb(255, 255, 255)' }}
                onMouseEnter={e => (e.currentTarget.style.color = '#dd1414')}
                onMouseLeave={e => (e.currentTarget.style.color = 'rgb(255, 255, 255)')}
              >{lbl}</Link>
            ))}
            <span className="hidden md:block w-px h-4" style={{ background: 'rgba(255,255,255,0.15)' }} />
      ...
      <nav className="bg-white w-full relative z-50 transition-shadow duration-300" style={{ boxShadow: scrolled ? '0 4px 24px rgba(22,72,120,0.12)' : '0 2px 8px rgba(22,72,120,0.06)' }}>
        ...
        <div className="max-w-[90rem] mx-auto px-6 lg:px-12 flex justify-between items-center h-[76px]">
          ...
          <div className="hidden lg:flex items-center h-full">
            ...
            {/* ── Correctly Stacked RFP/RFQ Buttons ── */}
