...
const navStructure: NavItem[] = [
  { label: 'HOME', href: '/' },
  {
    ...
    subItems: [
      ...
      { label: 'Our ESG Framework', href: '/about#esg' },
    ],
  },
  {
...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <header
        className={`w-full fixed top-0 left-0 right-0 z-[100] transition-transform duration-500 ease-in-out ${scrolled ? '-translate-y-0' : 'translate-y-0'
          }`}
      >
        {/* ── Top Bar ── */}
            ...
            <div className="flex items-center gap-3 sm:gap-4 lg:gap-5 ml-auto shrink-0">
              ...
              <div className="hidden md:flex items-center gap-2">
                {[
                  ...
                  <a key={s.label} href={s.href} target={s.href !== '#' ? "_blank" : undefined} rel={s.href !== '#' ? "noopener noreferrer" : undefined} aria-label={s.label}
        ...
        {/* ── Main Nav ── */}
        <nav
          className={`w-full relative z-50 transition-all duration-300 ${scrolled ? 'bg-white backdrop-blur-md shadow-[0_4px_24px_rgba(22,72,120,0.12)]' : 'bg-white shadow-[0_2px_8px_rgba(22,72,120,0.06)]'
            }`}
        >
          ...
          <div className="max-w-[90rem] mx-auto px-4 xl:px-12 flex justify-between items-center h-[76px]">
            ...
            <Link href="/" className="relative h-[55px] w-[210px] xl:h-[65px] xl:w-[290px] flex-shrink-0 block transition-all duration-300">
              <Image src="/images/Logo5.png" alt="CopterJet International" fill sizes="(max-width: 768px) 190px, (max-width: 1280px) 225px, 290px" className="object-contain object-left" priority />
            </Link>
            {/* Desktop Links */}
      ...
      <div className="fixed bottom-6 right-6 z-[90] flex flex-col items-end gap-4">
        ...
        <button
          onClick={scrollToTop}
          className={`w-12 h-12 bg-[#0a1e35] text-white hover:bg-copter-red rounded-sm flex items-center justify-center shadow-lg transition-all duration-500 ease-in-out ${scrolled ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'
            }`}
